...
-- invokes __inext
---@diagnostic disable-next-line: lowercase-global
---@type fun(t: any, i: any): function<function, ...>
function inext(t, i)
	local m = debug.getmetatable(t)
...
---@diagnostic disable-next-line: lowercase-global
---@type fun(t: any): function<any, any>, any, nil
...
---@type fun(fn: function, env: table): nil
...
---@type fun(list: table<integer, any>, pos: any, value?: any): nil
...
---@type fun(list: table<integer, any>, pos: integer): any
---@diagnostic disable-next-line: duplicate-set-field
...
do
	...
	---@type fun(list: table<any, any>, i?: integer, j?: integer): any
	---@diagnostic disable-next-line: duplicate-set-field
...
do
	...
	---@type fun(tbl: table<any, any>, sep?: string, i?: integer, j?: integer): string
	---@diagnostic disable-next-line: duplicate-set-field
...
---@type fun(data: any, meta?: table | metatable): table
...
function ModUtil.Callable.Get(obj)
	...
	while obj do
		...
		if not callableCandidateTypes[t] then
			return pobj
		end
...
---@type fun(tbl: table, map: function<any>): nil
...
---@type fun(target: table, data: table)
...
function ModUtil.Table.UnKeyed(tbl)
	...
	for k in pairs(tbl) do
		if type(k) ~= "number" or k > n or k < 1 or k ~= math.floor(k) then
			return false
		end
	end
...
---@type fun(sep?: string, ...): string
function ModUtil.String.Join(sep, ...)
	return table.rawconcat(table.pack(...), sep)
end

---@type fun(text: string, chunkSize: integer, maxChunks: integer): table<string>
...
---@type fun(state: string, start: integer, stop: integer, step: integer): string
function ModUtil.String.Slice(state, start, stop, step)
	if step == 1 then -- I like to assume using inbuilt functions is generally more efficient
		return state.sub(state, start, stop)
	end
...
function ModUtil.ToString.Address(o)
	local t = type(o)
	if t == "string" or passByValueTypes[t] then
		return nil
	end
...
---@type fun(obj: table): boolean
local function isNamespace(obj)
	return obj == _ENV_ORIGINAL
		or obj == _ENV_REPLACED
		or obj == objectData
		or ModUtil.Mods.Inverse[obj]
...
--]]
---@type fun(state: table<integer, any>, start?: integer, stop?: integer, step?: integer): table<integer, any>
...
--]]
---@type fun(baseTable: table<any, any>, indexArray: table<integer, integer>): table?
...
--]]
---@type fun(baseTable: table<any, any>, indexArray: table<integer, integer>, value: any): boolean | nil
...
---@type fun(baseTable: table<any, any>, indexArray: table<integer, integer>, map: function, ...): boolean | nil
...
function ModUtil.Path.Join(p, ...)
	...
	if p == '' then
		return ModUtil.Path.Join(q, ModUtil.Args.Drop(1, ...))
	end
	if q == '' then
		return ModUtil.Path.Join(p, ModUtil.Args.Drop(1, ...))
	end
...
function ModUtil.Path.IndexArray(path)
	if type(path) == "table" then
		return path
	end -- assume index array is given
	local s = ""
	local i = {}
...
ModUtil.Context.Env = ModUtil.Context(function(info)
	...
	info.env = setmetatable({}, {
		__index = function(_, key)
			...
			return rawget(env, key) or info.penv[key]
		end,
...
---@type fun(obtain: function): function
...
---@type fun(key: string | table, ignore: boolean): nil
...
--]]
---@type fun(first: string, second: table | nil, meta: boolean): table
function ModUtil.Mod.Register(first, second, meta)
	local modName_nil
	local parent
	if type(first) == "string" then
		modName_nil, parent = first, second
	else
		modName_nil, parent = second, first
	end
	local path

	---@type string | table
	local modName = modName_nil or ""
	if not parent then
		parent = _G
